...
async def on_ready():
    ...
    print(
        f'WaterBot Version {meta.version}.\n'
        f'Written by Alex Robinson.\n'
...
async def help(ctx):
    ...
    embed=discord.Embed(
        ...
        title="WaterBot Information & Help", 
        color=0x00ff00
        
    )

    embed.set_thumbnail(url=bot.user.avatar_url)
    embed.set_footer(text="A very watered bot.")
    ...
    embed.add_field(
        ...
        `invite` - Sends a link to invite WaterBot to your own server.
    ...
    embed.add_field(
        ...
        name="Invite WaterBot!",
        value="Want WaterBot in your own server? Click [here](https://discord.com/api/oauth2/authorize?client_id=812352738061975642&permissions=8&scope=bot)!",
        inline=True
...
async def info(ctx):
    ...
    embedvar.set_author(name=f"WaterBot")
    embedvar.add_field(
        name="\n:information_source: Information",
        value=f"Running Discord.py API Version **{discord.__version__}**\nFor a full list of commands, say `>help`\n ", inline=True
    )
    await ctx.send(embed=embedvar)
...
async def on_command_error(ctx, exception):
    ...
    if 'Command' in str(exception) and 'not found' in str(exception):
        ...
        await ctx.send(embed=discord.Embed(title="Something went wrong", description="Error information has been sent to the developer.", colour=0xff0000).add_field(name="Exception:",value=f'```{exception}```'))
